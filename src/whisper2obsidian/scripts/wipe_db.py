...
def main() -> None:
    ...
    if not db_path.exists() and not settings.chroma_db_dir.exists():
        console.print(f"[yellow]Databases not found at {db_path} or {settings.chroma_db_dir}[/yellow]")
        return

    console.print(f"[bold red]WARNING:[/] This will delete all tracking records in [cyan]{db_path}[/]")
    console.print(f"It will also wipe all semantic vector embeddings in [cyan]{settings.chroma_db_dir}[/]")
    console.print("This forces whisper2obsidian to completely re-process all audio files and re-index your entire vault next time it runs.")
    ...
    try:
        ...
        if settings.chroma_db_dir.exists():
            ...
            try:
                ...
                console.print("[bold green]✓ ChromaDB vector embeddings successfully wiped.[/bold green]")
            except ValueError:
                # Occurs if the collection does not exist
                console.print("[yellow]ChromaDB collection 'vault_notes' was already empty or did not exist.[/yellow]")
